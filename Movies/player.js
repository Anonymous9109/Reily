/* 
  Cyrene Player (Firebase Integrated)
  Smart source detection + Firebase Auto-Resume + Netflix UI Logic
*/
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const css = `
    ...
    #videoPlayer::after { content: ""; position: absolute; inset: 0; background: linear-gradient(to top, rgba(0,0,0,0.8) 0%, rgba(0,0,0,0.4) 20%, transparent 50%), linear-gradient(to bottom, rgba(0,0,0,0.5) 0%, transparent 30%); z-index: 20; pointer-events: none; opacity: 0; transition: opacity .4s ease; }
    #videoPlayer.show-ui::after { opacity: 1; }
    #subDisplay { position: absolute; bottom: 8%; left: 10%; right: 10%; text-align: center; z-index: 25; pointer-events: none; font-family: 'CyreneCustom', sans-serif; transition: bottom 0.3s cubic-bezier(0.4, 0, 0.2, 1); display: flex; justify-content: center; }
    #subDisplay span { padding: 0.2em 0.5em; border-radius: 0.2em; line-height: 1.2; color: #ffffff; font-size: 4.5vmin; text-align: center; transition: all 0.2s ease; }
    #videoPlayer.show-ui #subDisplay { bottom: 20%; }
    .controls { position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); display:flex; gap:90px; align-items:center; z-index:30; opacity:0; visibility:hidden; transition:opacity .2s, visibility .2s; }  
    ...
    #ccBtn { position: absolute; bottom: 25px; left: 5%; background: none; border: none; cursor: pointer; z-index: 35; opacity: 0; visibility: hidden; transition: opacity .2s, visibility .2s; display: none; align-items: center; }
    #ccBtn svg { width:32px; height:32px; fill:#fff; }
    #ccBtn.active svg { fill: red; }
    #sub-menu { position: absolute; bottom: 65px; left: 5%; background: rgba(15, 15, 15, 0.95); border: 1px solid #444; border-radius: 4px; padding: 5px 0; display: none; flex-direction: column; z-index: 50; min-width: 120px; }
    ...
    #backToPrev { position:absolute; top:14px; left:14px; width:46px; height:46px; border:none; border-radius:50%; background:rgba(0,0,0,.55); display:flex; align-items:center; justify-content:center; cursor:pointer; transition:background .25s, opacity .2s, visibility .2s; z-index:40; opacity:0; visibility:hidden; }  
    #backToPrev:hover { background:rgba(255,0,0,0.65); }  
    #backToPrev svg { width:28px; height:28px; fill:#fff; }  
    .progress-container { position:absolute; bottom:60px; left:5%; right:5%; height:20px; display:flex; align-items:center; cursor:pointer; z-index:25; opacity:0; visibility:hidden; transition:opacity .2s, visibility .2s; }  
    ...
    .progress-bar::after { content: ""; position: absolute; right: -8px; top: 50%; transform: translateY(-50%) scale(0); width: 16px; height: 16px; background: red; border-radius: 50%; transition: transform 0.1s ease; }
    .progress-container.dragging .progress-bar::after { transform: translateY(-50%) scale(1); }
    .video-timer { position: absolute; bottom: 85px; left: 5%; color: #fff; font-family: sans-serif; font-size: 14px; font-weight: 600; z-index: 25; opacity: 0; visibility: hidden; transition: opacity .2s, visibility .2s; }
    .nav-btn { position:absolute; top:16px; background:transparent; color:#fff; border:none; font-size:22px; padding:8px 12px; cursor:pointer; z-index:30; opacity:0; visibility:hidden; transition:opacity .2s, visibility .2s; }  
    .back-btn { left:12px; }  
    .next-btn { right:12px; }  
  ...
  /********** 2) Build DOM (Original Structure) **********/
  ...
  async function saveProgress(user, force = false) {
    ...
    // Save every 5 seconds OR if paused
  ...
  video.onplaying = () => { loadingRing.style.display = "none"; playIcon.style.display = "none"; pauseIcon.style.display = "block"; };
  ...
  window.onmouseup = () => { if (isDragging) { isDragging = false; showControls(); } };

  root.onclick = () => controlsVisible ? hideControls() : showControls();

  /********** 6) Orientation & Fullscreen **********/
  ...
  window.onresize = rotateIfPortrait;
  rotateIfPortrait();
  ...
  controls.className = "controls";
  controls.innerHTML = `
    <button id="rewindBtn"><svg width="50" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
